...
@author: sunday
"""
import sys,threading,time,re,urllib2,httplib,urlparse,random,socket
#for database
...
from gl_func import getPageEncoding,getLocalEncoding,isChildDomain,get_word_domain
#import gl_func

gl_var={ "sDeep":2,"DBFile":"D:\\Sunday\\hURL\\data\\global.db",
        "countN":10,"countM":100,"countO":500,"threadCount":1000,
        "IgList":[],'hURList':[],
        'DEBUG':True,"DBGFILE":".\\this.log",'logFile':'d:\\Sunday\\hURL\\other\scanner.log',
        #IE 7.0
        ...
        'finished':set(),                           #已被扫描结束的网站
        'plock':threading.Lock(),'myPrintHandle':sys.stdout,
        'PAGECOUNT_OF_CONNECTION':10, #平均每次连接，下载几个网页
        "hURLList":[],'version':'0.0.3'}


gl_getUserAgent = lambda : random.choice(gl_var['Agent'])
...
getGlobalFileListFromDB = lambda :gl_db.get_hURL()
getGlobalFileList = lambda :gl_var['hURList']
getScanList = lambda baseURL,fileList: \
    map(lambda el:(urlparse.urljoin(baseURL, \
    el[0]),el[1],el[2],el[3]),fileList)
netpage2localencode = lambda txt : txt.decode(getPageEncoding(txt),\
    errors='replace').encode(getLocalEncoding())
getIgnoreSiteFromDB   = lambda :gl_db.get_ignore()
getIgnoreSite         = lambda : gl_var['IgList']  

#getScanList =lambda word: [el for el in get_word_domain() if not needIgnore(el)]
...
class hURL(Base):
    ...
    CODE= Column(String)
    NOTE = Column(String)
    def __init__(self,FILENAME,FTYPE,CODE,NOTE):
        self.FILENAME=FILENAME                  #文件名
        self.FTYPE = FTYPE              #匹配模式
        self.CODE = CODE                        #关键词
        self.NOTE = NOTE                        #备注
    def __repr__(self):
        return "<hURL (%s,%s,%s,%s,%s)>"  \
            %(self.ID,self.FILENAME,self.CODE,self.FTYPE,self.NOTE)
...
class myDatabase():
    ...
    def add_hURL_one(self,filename,ftype,code,note):
        """添加一行记录到hURL"""
        self.session.add(self.hURL(FILENAME = filename,
                                FTYPE = ftype,
                                CODE = code,
                                NOTE = note))
    def add_hURL(self,hURL_li):
        """小黑们常用的URL"""
        for i,(filename,ftype,code,note) in enumerate(hURL_li):
            filename = filename.strip()
            code = code.strip()
            if ftype != 'normal' and ftype != 'regex':
                ftype = 'normal'
            ftype = ftype.strip()
            self.add_hURL_one(filename,ftype,code,note)
        try:
            self.commit()
    ...
    def get_hURL(self):
        ...
        for el in s.query(self.hURL):
            li.append( [el.FILENAME,el.FTYPE,el.CODE,el.NOTE] )
        return li
    def add_hURL_From_File(self,filename):
        ...
        assert len(txt[1]) == 4, "ERROR for your txt file"
        txt = map(lambda el: [el[0].strip(),el[1].strip(),
                    el[2].strip(),el[3].strip()] , txt)  #去掉空格
        txt = map(lambda el: [el[0],                    #Filename
                unicode(el[1],'UTF-8','ignore'),        #FType
                unicode(el[2],'UTF-8','ignore'),        #CODE
                unicode(el[3],'UTF-8','ignore') ],      #NOTE
                txt)
        ftxt.close()
        self.add_hURL(txt)
    ...
    def get_ignore(self):
        ...
        for site in s.query(self.IgnoreSite.SITE):
            li.append(site[0].decode('UTF-8').encode(getLocalEncoding()))
        return li
    def commit(self):
        """提交事务"""
        self.session.commit()
...
def needIgnore(site):
    """是否需要被忽略 """
    tmp_li = getIgnoreSite()
    #print tmp_li
    assert len(tmp_li) >=10
    for el in tmp_li:
        if isChildDomain(site,el):
            return True
    return False

def getSitelist(word):
    """"""
    tmp_li = get_word_domain(word)
    return [el for el in tmp_li if not needIgnore(el)]


def UnitTest():
    """单元测试"""
    li = getSitelist(raw_input('>'))
    for el  in li:
        print el
...
def init():
    ...
    global gl_var                               
    global gl_db 
    gl_db = myDatabase(gl_var['DBFile'])

    gl_var['IgList'] = gl_db.get_ignore()        #忽略站点列表
    gl_var['hURList']= getGlobalFileListFromDB()
    ...
    #myPrint('调试信息:\n\n\tlen(IgList)=%d'%len(gl_var['IgList']))
...
def _makeREQ(url):
    ...
    global gl_var
    req.add_header('User-Agent',gl_getUserAgent())
    return req
